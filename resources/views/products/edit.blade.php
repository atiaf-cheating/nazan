...
@section('content')
    ...
    @endphp
        ...
        @endif
        <form method="post" action="{{url('/products.update/'.$product->id)}}"  enctype="multipart/form-data" class=" @if($language == 'ar') rtl @endif" style="width: 90%;border-top: 4px solid #007BFF;padding: 3%; margin: 2%;background: white;">
            {{ csrf_field() }}
            ...
            <div class="form-group  @if($language == 'ar') rtl @endif">
                ...
                <select name="merchant_id" class="browser-default custom-select">
                    @foreach($merchants as $merchant)
                        <option value="{{$merchant->id}}" @if($product->merchant_id == $merchant->id) selected @endif>{{$merchant->arabic_name .'-'. $merchant->english_name}}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group @if($language == 'ar') rtl @endif">
