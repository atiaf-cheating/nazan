...
@section('content')
    ...
    @endif
    <form method="post" action="{{url('/merchant/control/products/color-size/store')}}" id="color_size_form" enctype="multipart/form-data" class=" @if($language == 'ar') rtl @endif"
...
{{--            @php dd($color_product) @endphp--}}
            <tr id="{{$key}}" class=" @if($language == 'ar') rtl @endif">
                ...
                <th class=" @if($language == 'ar') rtl @endif">
                    <a href="{{url('merchant/control/color_size/edit/'.$product_id.'/'.$color_product->id.'/'.$cat_id)}}" class="btn btn-info @if($language == 'ar') rtl @endif">@lang('messages.edit')</a>
                    ...
                    <a href="{{url('merchant/control/color_size/delete/'.$color_product->id)}}" onclick="return confirm('Are you sure?')" class="btn btn-danger @if($language == 'ar') rtl @endif">@lang('messages.delete')</a>
        ...
        // $( document ).ready(function() {
            ...
            function showSizes(key) {
                ...
                if ($('.sizesDetailedTable_' + key).length === 0) {
                    ...
                    $.ajax({
                        method: "get",
                        url: "merchant/control/color_size/sizes/" + $id,
                        data: {
                            id: $id,
                            "_token": "{{ csrf_token() }}"
                        }
            ...
            function deleteSizeFunction(btnKey, color_size_id) {
                ...
                $.ajax({
                    method: "post",
                    url: "/merchant/control/color_size/delete-price-quantity",
                    data: {
                        "id": color_size_id,
                        "_token": "{{ csrf_token() }}"
                    }
            ...
            function storeSizeDataFunction(btnKey, color_size_id, price, quantity, discount) {
                $.ajax({
                    method: "post",
                    url: "/merchant/control/color_size/edit-price-quantity",
                    data: {
                        "id": color_size_id,
            ...
            function deactivateSizeFunction(btnKey, color_size_id) {
                $.ajax({
                    method: "post",
                    url: "/merchant/control/color_size/deactivate-price-quantity",
                    data: {
                        "id": color_size_id,
                        "_token": "{{ csrf_token() }}"
                    }
